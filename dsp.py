...
class DSPMasterProgram:
    ...
    def calculate_z_transform(self):
        try:
            ...
            result = self.z_transform(sequence)
            ...
            self.z_output.insert(tk.END, f"Explanation:\n")
            self.z_output.insert(tk.END, f"The Z-transform converts a discrete sequence to frequency domain.\n")
            self.z_output.insert(tk.END, f"Each term represents: a[n] * z^(-n)")
        except ValueError:
            messagebox.showerror("Error", "Invalid input! Enter only numbers separated by spaces.")
    
    def z_transform(self, sequence):
        terms = []
    ...
    def create_windowing_tab(self):
        ...
        # Control Panel
        control_frame = ttk.LabelFrame(frame, text="Window Functions", padding="10")
        control_frame.pack(fill='x', pady=10)
        
        ttk.Button(control_frame, text="Rectangular", command=lambda: self.apply_window('Rectangular')).pack(side='left', padx=5)
        ttk.Button(control_frame, text="Blackman", command=lambda: self.apply_window('Blackman')).pack(side='left', padx=5)
        ttk.Button(control_frame, text="Hann", command=lambda: self.apply_window('Hann')).pack(side='left', padx=5)
        ttk.Button(control_frame, text="Hamming", command=lambda: self.apply_window('Hamming')).pack(side='left', padx=5)
        
        # Matplotlib canvas
        self.window_fig = Figure(figsize=(14, 5), dpi=80)
        self.window_canvas = FigureCanvasTkAgg(self.window_fig, master=frame)
    ...
    def apply_window(self, window_name):
        ...
        ax2.plot(n, xw, label=window_name)
        ax2.plot(n, w, '--', label='Window')
        ...
        ax3.plot(freq, np.abs(X), label="Original", linewidth=2)
        ax3.plot(freq, np.abs(Xw), label="Windowed", linewidth=2)
        ...
        self.window_fig.suptitle(f"{window_name} Window Analysis", fontsize=14, fontweight='bold')
